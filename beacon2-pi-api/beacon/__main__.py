...
import ssl
from beacon.auth.auth_handler import lsaai_auth_middleware, lsaai_dataset_permissions

# Enable or disable LSAAI authentication
LSAAI_AUTH_ENABLED = True
...
class Resultset(EndpointView):
    # Use LSAAI authentication if enabled, otherwise use the original dataset_permissions
    @lsaai_dataset_permissions if LSAAI_AUTH_ENABLED else dataset_permissions
...
async def create_api():# pragma: no cover
    # Setup middlewares - add LSAAI authentication middleware if enabled
    middlewares = [cors_middleware(origins=conf.cors_urls)]
    
    if LSAAI_AUTH_ENABLED:
        middlewares.append(lsaai_auth_middleware)
        LOG.info("LSAAI authentication middleware enabled")
